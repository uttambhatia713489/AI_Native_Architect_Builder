...
# \- assume `SLA\_BREACH`, Event Bus, scheduler, direct service invocation, or another mechanism unless the approved contract and active domain rules require it; or
...
# Checks not configured or not applicable to the active technology/domain are recorded as `NOT\_APPLICABLE` rather than skipped silently.
...
# \- the configured score is below the CONDITIONAL\_PASS threshold.
...
# \- the configured score is at or above the CONDITIONAL\_PASS threshold and below the PASS threshold; and

# \- only permitted non-blocking findings remain.

# 

# `CONDITIONAL\_PASS` is not a mandatory human intervention state unless the active governance policy explicitly configures human disposition. Routing after `CONDITIONAL\_PASS` is controlled by the harness.
