...
# \- issue a `PASS`, `CONDITIONAL\_PASS`, or `FAIL` verdict;
...
# 15\. Emit `STATUS: CONTRACT\_CHANGE\_REQUIRED` when a material contract change is required to satisfy the approved scope.
...
# Generator must not assert `PASS`, `CONDITIONAL\_PASS`, or `FAIL` as a routing marker.
